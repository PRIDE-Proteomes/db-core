...
--   at:        2016-08-18 11:50:52 BST
--   site:      Oracle Database 11g
--   type:      Oracle Database 11g


--The TABLESPACEs and the user are already created by the DBAs


-- CREATE TABLESPACE PRIDEPROT_IND
--   --  WARNING: Tablespace has no data files defined
--   NOLOGGING OFFLINE EXTENT MANAGEMENT LOCAL AUTOALLOCATE FLASHBACK ON ;
--
-- CREATE TABLESPACE PRIDEPROT_TAB
--   --  WARNING: Tablespace has no data files defined
--   NOLOGGING OFFLINE EXTENT MANAGEMENT LOCAL AUTOALLOCATE FLASHBACK ON ;

-- CREATE USER PRIDEPROT IDENTIFIED EXTERNALLY ACCOUNT UNLOCK ;

CREATE SEQUENCE PRIDEPROT.CLUSTER_PSM_PSM_ID_SEQ START WITH 1 INCREMENT BY 50000 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.FEATURE_FEATURE_ID_SEQ START WITH 1 INCREMENT BY 50 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.PEPTIDE_PEPTIDE_ID_SEQ START WITH 1 INCREMENT BY 1 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.Q_METHOD_Q_METHOD_ID_SEQ START WITH 1 INCREMENT BY 50 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.SCORE_SCORE_ID_SEQ START WITH 1 INCREMENT BY 50 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.STAR_STAR_ID_SEQ START WITH 1 INCREMENT BY 50 NOCACHE ORDER ;
...
CREATE TABLE PRIDEPROT.CLUSTER_PSM
  (
    ...
    ASSAY_ACCESSION NVARCHAR2 (50) CONSTRAINT NNC_CLUSTER_PSM_ASSAY_AC NOT NULL ,
    PROJECT_ACCESSION NVARCHAR2 (20) CONSTRAINT NNC_CLUSTER_PSM_PROJECT_AC NOT NULL ,
    CLUSTER_ID NUMBER CONSTRAINT NNC_CLUSTER_PSM_CLUSTER_ID NOT NULL
  )
  TABLESPACE PRIDEPROT_TAB ;
CREATE INDEX PRIDEPROT.CLUSTER_PSM_PSM_ID_IDX ON PRIDEPROT.CLUSTER_PSM
  (
    PSM_ID ASC
  )
  TABLESPACE PRIDEPROT_IND ;
CREATE INDEX PRIDEPROT.CLUSTER_PSM_SEQUENCE_IDX ON PRIDEPROT.CLUSTER_PSM
  ...
  TABLESPACE PRIDEPROT_IND ;
CREATE INDEX PRIDEPROT.CLUSTER_PSM_COMBI_IDX ON PRIDEPROT.CLUSTER_PSM
  (
    SEQUENCE DESC ,
    TAXID DESC ,
    MODS DESC ,
  ...
  TABLESPACE PRIDEPROT_IND ;
ALTER TABLE PRIDEPROT.CLUSTER_PSM ADD CONSTRAINT CLUSTER_PSM_PK PRIMARY KEY ( PSM_ID ) ;
...
CREATE TABLE PRIDEPROT.CV_PARAM
  ...
  TABLESPACE PRIDEPROT_TAB ;
ALTER TABLE PRIDEPROT.CV_PARAM ADD CONSTRAINT CK_CV_PARAM_CV_TYPE CHECK ( CV_TYPE IN ('CELL_TYPE', 'DISEASE', 'FEATURE_TYPE', 'TISSUE')) ;
...
CREATE UNIQUE INDEX PRIDEPROT.PEPTIDE_PEPTIDE_ID_IDX ON PRIDEPROT.PEPTIDE ( PEPTIDE_ID ASC ) TABLESPACE PRIDEPROT_IND ;
CREATE UNIQUE INDEX PRIDEPROT.PEPTIDE_REPRESENTATION_IDX ON PRIDEPROT.PEPTIDE ( REPRESENTATION ASC ) TABLESPACE PRIDEPROT_IND ;
...
CREATE UNIQUE INDEX PRIDEPROT.PEP_PROT_PK_IDX ON PRIDEPROT.PEP_PROT
  ...
  TABLESPACE PRIDEPROT_IND ;
CREATE UNIQUE INDEX PRIDEPROT.PEP_PROT_INV_PK_IDX ON PRIDEPROT.PEP_PROT
  (
    PEPTIDE_ID ASC , PROTEIN_ID ASC , START_POSITION ASC
  )
  TABLESPACE PRIDEPROT_IND ;
...
CREATE TABLE PRIDEPROT.PROTEIN
  (
    ...
    EVIDENCE    NUMBER ,
    ISOFORM     CHAR (1) DEFAULT 'F' NOT NULL ,
    CONTAMINANT CHAR (1) DEFAULT 'F' NOT NULL ,
    SCORE_ID    NUMBER DEFAULT 1 CONSTRAINT NNC_PROTEIN_SCORE_FK NOT NULL
  )
  TABLESPACE PRIDEPROT_TAB ;
ALTER TABLE PRIDEPROT.PROTEIN ADD CONSTRAINT CK_PROTEIN_ISOFORM CHECK ( ISOFORM         IN ('F', 'T')) ;
...
IS
  'Accession for Uniprot' ;
CREATE UNIQUE INDEX PRIDEPROT.PROTEIN_PROTEIN_ID_IDX ON PRIDEPROT.PROTEIN ( PROTEIN_ID ASC ) TABLESPACE PRIDEPROT_IND ;
...
CREATE TABLE PRIDEPROT.RELEASE_SUM
  (
    ...
    NUM_MAPPED_PROTS_W_PE1        NUMBER ,
    NUM_MAPPED_PROTS_W_PE2        NUMBER ,
    NUM_MAPPED_PROTS_W_PE3        NUMBER ,
    NUM_MAPPED_PROTS_W_PE4        NUMBER ,
    NUM_MAPPED_PROTS_W_PE5        NUMBER ,
    NUM_MAPPED_PROTS_W_PE_NA      NUMBER ,
    NUM_PROTS_W_PE1               NUMBER ,
    NUM_PROTS_W_PE2               NUMBER ,
    NUM_PROTS_W_PE3               NUMBER ,
    NUM_PROTS_W_PE4               NUMBER ,
    NUM_PROTS_W_PE5               NUMBER ,
    NUM_PROTS_W_PE_NA             NUMBER
  )
  TABLESPACE PRIDEPROT_TAB ;
...
ALTER TABLE PRIDEPROT.FEATURE ADD CONSTRAINT FEATURE_CV_PARAM_FK FOREIGN KEY ( CV_TERM ) REFERENCES PRIDEPROT.CV_PARAM ( CV_TERM ) NOT DEFERRABLE ;

ALTER TABLE PRIDEPROT.FEATURE ADD CONSTRAINT FEATURE_PROTEIN_FK FOREIGN KEY ( PROTEIN_ID ) REFERENCES PRIDEPROT.PROTEIN ( PROTEIN_ID ) NOT DEFERRABLE ;
...
--   PEP_UNIQ.UNIQUENESS,
--   PROT_GROUP.PROT_GROUP_TYPE AS PROT_GROUP_TYPE,
--   PROT_PGRP.PROT_GROUP_ID AS PROT_GROUP_ID,
--   PROT_GROUP.TAXID AS TAXID
-- FROM
--   PEPTIDE,
--   PEP_PROT,
--   PROT_PGRP,
...
--   (SELECT
--       COMPACT.PEPTIDE_ID,
--       COUNT(COMPACT.PEPTIDE_ID) UNIQUENESS,
--       COMPACT.PROT_GROUP_TYPE
--   FROM
--     (SELECT DISTINCT
--       PEPTIDE.PEPTIDE_ID AS PEPTIDE_ID,
--       PROT_GROUP.PROT_GROUP_ID,
--       PROT_GROUP.PROT_GROUP_TYPE AS PROT_GROUP_TYPE
--     FROM
--       PEPTIDE,
--       PEP_PROT,
...
--   AND PROT_PGRP.PROT_GROUP_ID = PROT_GROUP.PROT_GROUP_ID
--   AND PEPTIDE.SYMBOLIC = 'TRUE'
--   AND PEP_UNIQ.PEPTIDE_ID = PEPTIDE.PEPTIDE_ID
--   AND PEP_UNIQ.PROT_GROUP_TYPE = PROT_GROUP.PROT_GROUP_TYPE
-- ORDER BY PEPTIDE_ID,PROT_GROUP_TYPE;
