...
ALTER TABLE PRIDEPROT.PROT_GROUP ADD CHECK ( PROT_GROUP_TYPE IN ('ENTRY', 'GENE')) ;
...
SELECT DISTINCT PEP_UNIQ.PEPTIDE_FK_PK,
  PEP_UNIQ.UNIQUENESS,
  PROT_PGRP.P_GROUP_FK_PK
FROM
  (SELECT PEP_GROUP.PEPTIDE_PK  AS PEPTIDE_FK_PK,
    COUNT(PEP_GROUP.PEPTIDE_PK) AS UNIQUENESS
  FROM
    (SELECT PEPTIDE.PEPTIDE_PK,
      PROT_PGRP.P_GROUP_FK_PK
    FROM PEPTIDE,
      PROT_PEP,
      PROT_PGRP
...
CREATE SEQUENCE PRIDEPROT.PEPTIDE_PEPTIDE_PK_SEQ START WITH 1 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.Q_METHOD_QUALITY_METHOD_PK_SEQ START WITH 1 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.SCORE_SCORE_PK_SEQ START WITH 1 NOCACHE ORDER ;

CREATE SEQUENCE PRIDEPROT.STAR_STAR_PK_SEQ START WITH 1 NOCACHE ORDER ;
